...
lgrad <- function(f, x, grad.max) {
  grad.x <- grad(f, x);
  grad.x.magnitude <- sqrt(sum(grad.x^2));
  if(grad.x.magnitude > grad.max) {
    grad.x <- grad.x / grad.x.magnitude;
  }
  return(grad.x)
}
```
...
```{r}
library(numDeriv)
polak.ribiere <- function(f, x, tol, grad.max) {
  beta <- 1;
  ...
  repeat {
    ...
    grad.x <- grad(f, x)
    cat(grad.x, "\n");
    step <- golden(g,-1/d.magnitude ,1/d.magnitude ,tol)
    new.x <- x + step * d
    grad.new.x <- grad(f, new.x)
    cat("x=", x,"step=",step, "new.x=", new.x, "grad.x=", grad.x, "d=", d, "\n");
...
```{r}
m <- polak.ribiere(fn, c(-0.5, -1.0), 1e-3, 0.5)
m
fn(m)
```
