...
# Metoda Fibonacciego implementacja
...
# Rozwiązanie - metoda Fibonacciego
...
**algorytmu bisekcji** wybieramy punkt $c = \frac{a+b}{2}$, oraz obliczamy $f_c=f(c)$.
Następnie wybieramy ten z przedziałów $[a, m]$, $[m, b]$, dla których iloczyn $f(a)*f(m)$ 
lub $f(b)*f(m)$ jest *ujemny* (co oznacza, że minimum jest w wybranym przedziale).
Możemy wykorzystać ten fakt, aby znajdować minima/maxima funkcji tj. będziemy 
poszukiwać miejsc zerowych pochodnej.

# Poszukiwanie pochodnej funkcji

Ponieważ funkcja ta działa na liczbach zespolonych i oblicza *abs*, R nie
...
# Wizualizacja - bisekcja - animacja 
\centering 
![](img/bi/0.png)
...
metodą analityczną. Jednak jest to niemożliwe w przypadku tej funkcji gdyż wykorzystuje
ona funkcję **abs**.
